            ...
            <div class="d-flex align-items-center w-100">
                <div class="flex-grow-1">
                    <h5 class="modal-title mb-0 text-dark">
                        <i class="fas fa-exclamation-triangle me-2 text-danger"></i>Kesalahan
                    </h5>
            ...
            <div class="alert alert-danger bg-danger bg-opacity-10 border-danger border-opacity-25">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <p class="mb-0">Data kategori kerusakan yang anda cari tidak ditemukan</p>
                    </div>
                </div>
            </div>
            <div class="text-end">
                <a href="{{ url('/kategoriKerusakan') }}" class="btn btn-warning rounded-pill px-4">
...
@else
<form action="{{ url('/kategoriKerusakan/' . $kategoriKerusakan->kategori_kerusakan_id.'/delete') }}" method="POST" id="form-delete">
    @csrf
    @method('DELETE')
        ...
        <div class="modal-content border-0" style="border-radius: 15px; overflow: hidden; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.15);">
            <div class="modal-header text-white bg-light">
                <div class="d-flex align-items-center w-100">
                    <div class="flex-grow-1">
                        <h5 class="modal-title mb-0 text-dark">
                            <i class="fas fa-trash-alt me-2"></i>Hapus Data Kategori Kerusakan
                        </h5>
                        <p class="mb-0 small opacity-85 mt-1 text-muted">Konfirmasi penghapusan data kategori</p>
                    </div>
                    <button type="button" class="btn-close btn-close-dark" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
            </div>
                ...
                <div class="alert alert-warning bg-warning bg-opacity-10 border-warning border-opacity-25 mb-4">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <p class="mb-0">Apakah Anda yakin ingin menghapus data kategori kerusakan berikut?</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body p-3 bg-light" style="border-radius: 12px;">
                        <table class="table table-borderless table-sm mb-0">
                            <tr>
                                ...
                                <td style="width: 120px; padding: 0.5rem; vertical-align: middle;" class="text-muted small">ID Kategori</td>
                                <td style="padding: 0.5rem; vertical-align: middle;" class="fw-bold">
                                    <span class="badge bg-primary bg-opacity-10 text-primary">{{ $kategoriKerusakan->kategori_kerusakan_id }}</span>
                                </td>
                            </tr>
                            <tr>
                                <td style="width: 40px; padding: 0.5rem; vertical-align: middle; text-align: center;" class="text-primary">
                                ...
                                <td style="width: 120px; padding: 0.5rem; vertical-align: middle;" class="text-muted small">Nama Kategori</td>
                                <td style="padding: 0.5rem; vertical-align: middle;" class="fw-bold">
                                    {{ $kategoriKerusakan->nama_kategori }}
                                </td>
                            </tr>
                        </table>
                    </div>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status) {
                            ...
                            setTimeout(() => {
                                if (typeof dataKategori !== 'undefined') dataKategori.ajax.reload();
                            }, 1500);
                        } else {
                            Swal.fire({
                                icon: 'error',
