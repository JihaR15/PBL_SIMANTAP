                ...
                <div class="d-flex align-items-center w-100">
                    <div class="flex-grow-1">
                        <h5 class="modal-title mb-0 text-dark">
                            <i class="fas fa-user-cog me-2"></i>Tambah Data Role
                        </h5>
                        <p class="mb-0 small opacity-85 mt-1 text-muted">Formulir penambahan role baru</p>
                    </div>
    ...
    $(document).ready(function () {
        $('#form-create').on('submit', function (e) {
            ...
            if (!kodeRole) {
                ...
                $('#error-kode_role').text('Kode role wajib diisi');
            } else if (kodeRole.length < 2) {
                isValid = false;
                $('#kode_role').addClass('is-invalid');
                $('#error-kode_role').text('Minimal 2 karakter');
            } else if (kodeRole.length > 10) {
                isValid = false;
                $('#kode_role').addClass('is-invalid');
                $('#error-kode_role').text('Maksimal 10 karakter');
            } else if (!/^[A-Z0-9_]+$/i.test(kodeRole)) {
                isValid = false;
                $('#kode_role').addClass('is-invalid');
                $('#error-kode_role').text('Hanya boleh huruf, angka dan underscore');
            }

            if (!namaRole) {
                isValid = false;
                $('#nama_role').addClass('is-invalid');
                $('#error-nama_role').text('Nama role wajib diisi');
            } else if (namaRole.length < 3) {
                isValid = false;
                $('#nama_role').addClass('is-invalid');
                $('#error-nama_role').text('Minimal 3 karakter');
            } else if (namaRole.length > 50) {
                isValid = false;
                $('#nama_role').addClass('is-invalid');
                $('#error-nama_role').text('Maksimal 50 karakter');
            }
