...
<div id="modal-master" class="modal-dialog" role="document">
    <div class="modal-content border-0" style="border-radius: 15px; overflow: hidden; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.15);">
        ...
        <div class="modal-body p-4">
            ...
            <div class="text-end">
                <a href="{{ url('/jenisbarang') }}" class="btn btn-warning rounded-pill px-4">
...
@else
<form action="{{ url('/jenisbarang/' . $jenisbarang->jenis_barang_id.'/delete') }}" method="POST" id="form-delete">
    @csrf
    @method('DELETE')
        ...
        <div class="modal-content border-0" style="border-radius: 15px; overflow: hidden; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.15);">
            <div class="modal-header text-white bg-light">
                <div class="d-flex align-items-center w-100">
                    <div class="flex-grow-1">
                        <h5 class="modal-title mb-0 text-dark">
                            <i class="fas fa-trash-alt me-2"></i>Hapus Data Jenis Barang
                        </h5>
                        <p class="mb-0 small opacity-85 mt-1 text-muted">Konfirmasi penghapusan data barang</p>
                    </div>
                    <button type="button" class="btn-close btn-close-dark" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
            </div>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status) {
                            ...
                            setTimeout(() => {
                                if (typeof dataBarang !== 'undefined') dataBarang.ajax.reload();
                            }, 1500);
                        } else {
                            Swal.fire({
                                icon: 'error',
